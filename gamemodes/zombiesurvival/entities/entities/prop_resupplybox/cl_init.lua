...
function ENT:RenderInfo(pos, ang, owner)
	cam.Start3D2D(pos, ang, 0.075)
		...
		draw.SimpleText(translate.Get("resupply_box") .. " " .. timertext, "ZS3D2DFont2", 0, -50, timercolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

		if owner:IsValid() and owner:IsPlayer() then
			draw.SimpleText("("..owner:ClippedName()..")", "ZS3D2DFont2Small", 0, 65, owner == MySelf and COLOR_BLUE or COLOR_GRAY, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
		end
...
function ENT:Draw()
	...
	cam.Start3D2D(self:LocalToWorld(vOffsetEE), ang, 0.01)
		draw.SimpleText("meow--", "ZS3D2DFont2", 0, 0, color_white, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
